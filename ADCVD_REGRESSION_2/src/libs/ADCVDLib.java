...
import static GuiLibs.GuiTools.elementEnabled;
import static GuiLibs.GuiTools.clickNiemElementJs;
import static GuiLibs.GuiTools.elementExists;
import static GuiLibs.GuiTools.getElementListAttribute;
...
public class ADCVDLib{
	...
	public static String orderId, adcvdCaseName, adCaseId, cvdCaseId, arSegmentId;
	...
	*/
	...
	/**
	 * This method checks product manager creates new ADCVD case
	 * @param row: map of test case's data
	 * @return true case created correctly, false if not
	 * @exception Exception
	*/
	public static boolean CheckProgrmManagerCreateNewCase(LinkedHashMap<String, String> row, 
															boolean pmExists) throws Exception
	{	
		pageRefresh();
		holdSeconds(2);
		clickElementJs(guiMap.get("adcvdCasesObjectLink"));
		caseType =  row.get("ADCVD_Case");
		String programManager = row.get("Program_Manager"); 
		holdSeconds(2);
		clickNiemElementJs(guiMap.get("newCaseLink"), 1);
		int waitTime = setBrowserTimeOut(2);
		setBrowserTimeOut(waitTime);
		String recType = "recTypeFiled";
		if(row.get("Record_Type").equalsIgnoreCase("Self-Initiate"))
		recType = "recTypeSelfInitiated";
		clickElementJs(guiMap.get(recType));
		...
		if(pmExists)
		{
			if(!elementExists(guiMap.get("ProgramManager")))
			{
				failTestCase("Check Program manager field", "Program Manager field should display",
					"Not as expected", "VP", "fail", "ProgManager");
			}
			else
			{
				highlightElement(guiMap.get("ProgramManager"), "green");
				updateHtmlReport("Check Program manager field", "Program Manager field should display",
						"As expected", "VP", "pass", "ProgManager");
			}
		...
		}else
		{
			if(elementExists(guiMap.get("ProgramManager")))
			{
				highlightElement(guiMap.get("ProgramManager"), "red");
				failTestCase("Check Program manager field", "Program Manager field should not display",
					"Not as expected", "VP", "fail", "ProgManager");
				unHighlightElement(guiMap.get("ProgramManager"));
			}
			else
			{
				updateHtmlReport("Check Program manager field", "Program Manager field should not display",
						"As expected", "VP", "pass", "ProgManager");;
			}
		}
		...
		if(pmExists)
		{
			enterText(guiMap.get("ProgramManager"), programManager);
			waitTime = setBrowserTimeOut(2);
			if(elementExists(replaceGui(guiMap.get("ProgramManagerVal"), programManager)))
			{
				clickElementJs(replaceGui(guiMap.get("ProgramManagerVal"), programManager));
				highlightElement(guiMap.get("ProgramManager"), "green");
				updateHtmlReport("Search Program Manager", "Program Manager ["+programManager+"] should be found",
						"As expected", "VP", "pass", "ProgManager search");
				unHighlightElement(guiMap.get("ProgramManager"));
			}
			else
			{
				failTestCase("Search Program Manager", "Program Manager ["+programManager+"] should be found",
						"Not as expected", "VP", "fail", "ProgManager search");
			}
			setBrowserTimeOut(waitTime);
		}
		clickElementJs(guiMap.get("saveCaseButton"));
	...
	/**
	 * This method checks the lookup program manager
	 * @param row: map of test case's data
	 * @return true case created correctly, false if not
	 * @exception Exception
	*/
	public static boolean checkProgramManagerLookUp(LinkedHashMap<String, String> row, 
															boolean pmFound) throws Exception
	{	
		boolean lookup = true;
		pageRefresh();
		holdSeconds(6);
		...
		clickNiemElementJs(guiMap.get("newCaseLink"), 1);
	/*	holdSeconds(1);
		clickElement(guiMap.get("newCaseLink"));*/
		String recType = "recTypeFiled";
		if(row.get("Record_Type").equalsIgnoreCase("Self-Initiate"))
		recType = "recTypeSelfInitiated";
		holdSeconds(2);
		...
		enterText(guiMap.get("ProgramManager"), programManager);
		holdSeconds(1);
		clickElementJs(guiMap.get("ProgramManager"));
		scrollToTheBottomOfPage();
		int waitTime = setBrowserTimeOut(4);
		if(pmFound)
		{
			if(elementExists(replaceGui(guiMap.get("ProgramManagerVal"), programManager)))
			{
				clickElementJs(replaceGui(guiMap.get("ProgramManagerVal"), programManager));
				highlightElement(guiMap.get("ProgramManager"), "green");
				updateHtmlReport("Program manager look up", "Program Manager ["+programManager+"] should be found",
						"As expected", "VP", "pass", "Program manager look up");
				unHighlightElement(guiMap.get("ProgramManager"));
			}
			else
			{
				lookup = false;
				updateHtmlReport("Program manager look up", "Program Manager ["+programManager+"] should be found",
						"Not as expected", "VP", "fail", "Program manager look up");
			}
		}
		else // not to be found
		{
			if(elementExists(replaceGui(guiMap.get("ProgramManagerVal"), programManager)))
			{
				lookup = false;
				clickElementJs(replaceGui(guiMap.get("ProgramManagerVal"), programManager));
				highlightElement(guiMap.get("ProgramManager"), "red");
				updateHtmlReport("Program manager look up", "Program Manager ["+programManager+"] should not be found",
						"Not as expected", "VP", "fail", "Program manager look up");
				unHighlightElement(guiMap.get("ProgramManager"));
			}
			else
	...
	*/
	 ...
	 * @exception Exception
	*/
	public static boolean createNewOrderFromInvestigation (LinkedHashMap<String, String> row) throws Exception
	{
		holdSeconds(2);
		clickElementJs(guiMap.get("NewOrdertab"));
		holdSeconds(1);
		clickElementJs(guiMap.get("investOrderNew"));
		holdSeconds(1);
		clickElementJs(guiMap.get("investOrderSave"));
		holdSeconds(2);
		int currentTimeOut = setBrowserTimeOut(3);
		if(checkElementExists(guiMap.get("newCreateError")))
		{
		...
		}else
		{
			setBrowserTimeOut(currentTimeOut);
			holdSeconds(2);
			if(checkElementExists(guiMap.get("OrderNewCreated")))
			{
				orderId = getElementAttribute(guiMap.get("OrderNewCreated"), "text");
				//scrollToElement(guiMap.get("newCreatedOrder"));
				highlightElement(guiMap.get("OrderNewCreated"), "green");
				updateHtmlReport("Create order", "User is able to create a new order", 
						"Id: <span class = 'boldy'>"+" "
						+ ""+orderId+"</span>", "Step", "pass", "Ceate a new order");
				clickElementJs(guiMap.get("OrderNewCreated"));
				pageRefresh();
				return true;
			}else
			{
	...
	*/
	public static boolean createNewPetition (LinkedHashMap<String, String> row) throws Exception
	{
		...
		holdSeconds(3);
		int currentTimeOut = setBrowserTimeOut(5);
		if(checkElementExists(guiMap.get("newCreateError")))
		{
			highlightElement(guiMap.get("newCreateError"), "red");
			holdSeconds(5);
		...
		else
		{
			...
			}else
			{
				...
				scrollToElement(guiMap.get("linkNewPetition"));
	...
	/**
	 * This method validates program manager transferred
	 * @return true if the order is correct, false if not
	 * @exception Exception
	*/
	public static boolean validateProgramManagerTransferred(String pgrmManager) throws Exception
	{
		boolean transferred = true;
		scrollToElement(guiMap.get("staffOrderingDiv"));
		int currentWait = setBrowserTimeOut(2);
		
		
		if(elementExists(guiMap.get("StaffArea")))
		{
			highlightElement(guiMap.get("StaffArea"), "green");
			updateHtmlReport("Verify Staffing Lightning Web Component", "Staffing Lightning"
					+ " Web Component Exists", "As Expected", "VP", "pass", "Staffing Lightning Web Component");
		}else 
		{
			transferred = false;
			highlightElement(guiMap.get("StaffArea"), "green");
			updateHtmlReport("Verify Staffing Lightning Web Component", "Staffing Lightning"
					+ " Web Component Exists", "Not as Expected", "VP", "fail", "Staffing Lightning Web Component");
		}
		
		
		if(elementExists(replaceGui(guiMap.get("progMngrTransferred"), pgrmManager)))
		{
			highlightElement(replaceGui(guiMap.get("progMngrTransferred"), pgrmManager), "green");
			updateHtmlReport("Check if program manager transferred", "the program"
					+ " manager ["+pgrmManager+"] has been transferred", 
					"As Expected", "VP", "pass", "Program Manager transfered");
		}else 
		{
			transferred = false;
			highlightElement(replaceGui(guiMap.get("staffTitle"), "Program Manager"), "red");
			updateHtmlReport("Check if program manager transferred", "the program"
					+ " manager ["+pgrmManager+"] has been transferred", 
					"Not as Expected", "VP", "fail", "Program Manager transfered");
		}
		setBrowserTimeOut(currentWait);
		return transferred;
	}
	...
	/**
	 * This method validates staffing order
	 * @return true if the order is correct, false if not
	 * @exception Exception
	*/
	public static boolean validateStaffingTitleOrder(String stffOrder) throws Exception
	{
		boolean inOrder = true;
		String[] staffingOrder = stffOrder.split(",");
		String [] actualList = getElementListAttribute(guiMap.get("staffOrdering"), "text");
		scrollToElement(guiMap.get("staffOrderingDiv"));
		if(Arrays.equals(staffingOrder, actualList))
		{
			highlightElement(guiMap.get("staffOrderingDiv"), "green");
			updateHtmlReport("Check staff title order", "The order should be as : ["+stffOrder+"]", 
					"Actual Order is ["+actualList.toString()+"]", "VP", "pass", "Staffing title order");
			unHighlightElement(guiMap.get("staffOrderingDiv"));
		}else
		{
			inOrder = false;
			highlightElement(guiMap.get("staffOrderingDiv"), "red");
			updateHtmlReport("Check staff title order", "The order should be as : ["+stffOrder+"]", 
					"Actual Order is ["+actualList.toString()+"]", "VP", "fail", "Staffing title order");
			unHighlightElement(guiMap.get("staffOrderingDiv"));
		}
	...
	/**
	 * This method validates Customs tab label
	 * @return true if the order is correct, false if not
	 * @exception Exception
	*/
	public static boolean validateCustomTabText(String customTabText) throws Exception
	{
		boolean correct = true;
		holdSeconds(3);
		String currentName = getElementAttribute(guiMap.get("CustomTabLink"), "text");
		if(currentName.equalsIgnoreCase(customTabText))
		{
			highlightElement(guiMap.get("CustomTabLink"), "green");
			updateHtmlReport("Custom tab text", "The expected label of customs tab is: ["+customTabText+"] "
					+ "and the actual is ["+currentName+"]", 
					"As Expected", "VP", "pass", "Customs label");
			unHighlightElement(guiMap.get("CustomTabLink"));
		}else
		{
			correct = false;
			highlightElement(guiMap.get("CustomTabLink"), "red");
			updateHtmlReport("Custom tab text", "The expected label of customs tab is: ["+customTabText+"] "
					+ "and the actual is ["+currentName+"]", 
					"Not as Expected", "VP", "fail", "Customs label");
			unHighlightElement(guiMap.get("CustomTabLink"));
		}
	...
	/**
	 * This method validates Program Manager editable from staffing
	 * @return true if the order is correct, false if not
	 * @exception Exception
	*/
	public static boolean validateProgManagerWithRecusalEditable(String progManager, boolean linkEnabled) throws Exception
	{
		boolean correct = true;
		pageRefresh();
		holdSeconds(5);
		if(!linkEnabled)
		{
			if(!elementEnabled(guiMap.get("PreviewStaffManagerIcon")))
			{
				highlightElement(guiMap.get("PreviewStaffManagerIcon"), "green");
				updateHtmlReport("Check prog manager link not editable", "The program manager without"
						+ " Recusal is disabled: ["+progManager+"] ",
						"As Expected", "VP", "pass", "prog manager without recusal");
				unHighlightElement(guiMap.get("PreviewStaffManagerIcon"));
				
			}else
			{
				correct = false;
				highlightElement(guiMap.get("PreviewStaffManagerIcon"), "red");
				updateHtmlReport("Check prog manager link editable", "The program manager without"
						+ " Recusal is disabled: ["+progManager+"] ",
						"Not as Expected", "VP", "fail", "prog manager without recusal");
				unHighlightElement(guiMap.get("PreviewStaffManagerIcon"));
			}
		}else
		{
			...
			{
				highlightElement(guiMap.get("PreviewStaffManagerIcon"), "green");
				updateHtmlReport("Check prog manager link editable", "The program manager linked with"
						+ " Recusal is editable: ["+progManager+"] ",
						"As Expected", "VP", "pass", "prog manager with recusal");
				unHighlightElement(guiMap.get("PreviewStaffManagerIcon"));
				clickElementJs(guiMap.get("PreviewStaffManagerIcon"));
				String handler = switchToWindow(); 
				updateHtmlReport("Edited Recusal", "Recusal window should opens ",
						"As Expected", "VP", "pass", "Recusal window");
				switchBackToWindow(handler);
			...
			}else
			{
				...
				updateHtmlReport("Check prog manager link editable", "The program manager linked with"
						+ " Recusal is editable: ["+progManager+"] ",
						"Not as Expected", "VP", "fail", "prog manager with recusal");
				unHighlightElement(guiMap.get("PreviewStaffManagerIcon"));
			}
		}
		return correct;
	 ...
	 * @exception Exception
	*/
	public static boolean validateNewStaffCreation(String pgmMgr) throws Exception
	{
		boolean creation = true;
		scrollToElement(guiMap.get("staffOrderingDiv"));
		clickElementJs(guiMap.get("newStaffButton"));
		holdSeconds(2);
		clickElementJs(guiMap.get("staffSaveButton"));
		int currentWait = setBrowserTimeOut(2);
		if(elementExists(guiMap.get("staffselectTitleErr")))
		{
			highlightElement(guiMap.get("staffselectTitleErr"), "green");
			updateHtmlReport("Verify that title is required", "Error message about missing title should appear", 
					"As Expected", "VP", "pass", "Staffing assign missing title");
		}else 
		{
			creation = false;
			updateHtmlReport("Verify that title is required", "Error message about missing title should appear", 
					"Not as Expected", "VP", "fail", "Staffing assign missing title");
		}
		clickElementJs(guiMap.get("staffAddTitle"));
		clickElementJs(replaceGui(guiMap.get("staffAssigItem"), "Analyst"));
		enterText(guiMap.get("staffAddUser"), pgmMgr);
		clickElementJs(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr));
		holdSeconds(1);
		clickElementJs(guiMap.get("staffSaveButton"));
		if(elementExists(guiMap.get("staffAssCreatedSuccess")))
		{
			highlightElement(guiMap.get("staffAssCreatedSuccess"), "green");
			holdSeconds(2);
			updateHtmlReport("Create new staff", "User is able to create new staff", 
					"As Expected", "VP", "pass", "New staff created");
		}else 
		{
			creation = false;
			updateHtmlReport("Create new staff", "User is able to create new staff", 
					"Not as Expected", "VP", "fail", "New staff created");
		}
		return creation;
	}
	 ...
	 * @exception Exception
	*/
	public static boolean validateClearProgramManagerValue(String pgmMgr) throws Exception
	{
		boolean creation = true;
		scrollToElement(guiMap.get("staffOrderingDiv"));
		clickElementJs(guiMap.get("EditStaffManagerIcon"));
		holdSeconds(1);
		clickElementJs(guiMap.get("EditStaffManagerLink"));
		holdSeconds(1);
		clickElementJs(guiMap.get("EditProgManagerClear"));
		holdSeconds(1);
		clickElementJs(guiMap.get("staffSaveButton"));
		...
		int currentWait = setBrowserTimeOut(2);
		if(elementExists(guiMap.get("staffselectUserErr")))
		{
			highlightElement(guiMap.get("staffselectUserErr"), "green");
			updateHtmlReport("Verify that title is required", "Error message about missing user should appear", 
					"As Expected", "VP", "pass", "Staffing assign missing title");
		}else 
		{
			creation = false;
		...
		/*clickElementJs(guiMap.get("staffAddTitle"));
		clickElementJs(replaceGui(guiMap.get("staffAssigItem"), "Analyst"));
		enterTextAndClear(guiMap.get("staffAddUser"), pgmMgr);
		clickElementJs(replaceGui(guiMap.get("ProgramManagerVal"), pgmMgr));h
		clickElementJs(guiMap.get("staffSaveButton"));
		if(elementExists(guiMap.get("staffAssCreatedSuccess")))
		{
			highlightElement(guiMap.get("staffAssCreatedSuccess"), "green");
		...
		clickElementJs(guiMap.get("staffCancelButton"));
		holdSeconds(2);
		setBrowserTimeOut(currentWait);
		return creation;
	}
	...
	/**
	 * This method validates staffing prog manager lookup value based on its role
	 * @return true if the order is correct, false if not
	 * @exception Exception
	*/
	public static boolean validateSearchStaffBasedOnUserRole(String pgmMgr, String pgmMgrRole, boolean searched) throws Exception
	{
		boolean cantBeSearched = true;
		pageRefresh();
		holdSeconds(3);
		scrollToElement(guiMap.get("staffOrderingDiv"));
		clickElementJs(guiMap.get("EditStaffManagerIcon"));
		...
		if(!searched)
		{
			if(!elementExists(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr)))
			{
				highlightElement(guiMap.get("staffAddUser"), "green");
				updateHtmlReport("The program manager with a role ["+pgmMgrRole+"]", 
						"Program manager should not be able to be found", "As expected", "VP", 
						"pass", "Program manager look up");
				unHighlightElement(guiMap.get("staffAddUser"));
			}
			else
			{
				cantBeSearched = false;
				highlightElement(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr), "red");
				updateHtmlReport("The program manager with a role ["+pgmMgrRole+"]", 
						"Program manager should not be able to be found", "Not as expected", "VP", 
						"fail", "Program manager look up");
				unHighlightElement(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr));
			}
		}else
		{
			if(elementExists(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr)))
			{
				highlightElement(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr), "green");
				updateHtmlReport("The program manager with a role ["+pgmMgrRole+"]", 
						"Program manager should be able to be searched", "As expected", "VP", 
						"pass", "Program manager look up");
				unHighlightElement(replaceGui(guiMap.get("EditProgManagerValue"), pgmMgr));
			...
			{
				cantBeSearched = false;
				highlightElement(guiMap.get("staffAddUser"), "red");
				updateHtmlReport("The program manager with a role ["+pgmMgrRole+"]", 
						"Program manager should be able to be searched", "Not as expected", "VP", 
						"fail", "Program manager look up");
				unHighlightElement(guiMap.get("staffAddUser"));
			}
		}
	...
	*/
	public static boolean createNewInvestigation (LinkedHashMap<String, String> row) throws Exception
	{
		...
		enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Period Covered - Start Date"),
				new SimpleDateFormat("yyyy-MM-dd-HH.mm.ss").format(new Date()));
		enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Period Covered - End Date"), 
				new SimpleDateFormat("yyyy-MM-dd-HH.mm.ss").format(new Date()));
		clickElementJs(guiMap.get("saveCaseButton"));
		holdSeconds(6);
	 ...
	 * @exception Exception
	*/
	public static boolean createNewInvestigationFromPetition (LinkedHashMap<String, String> row) throws Exception
	{
		// pageRefresh();
		holdSeconds(2);
		//clickElementJs(guiMap.get("linkInvestigation"));
		//updateHtmlReport("Start creating new investigation", "User is able to click on investigation tab", 
				//"As expected", "Step", "pass", "Investigation tab");
		clickElementJs(guiMap.get("petitionInvestigationLink"));
		holdSeconds(2); 
		clickElementJs(guiMap.get("petitionInvestigationNew"));
		if(caseType.equals("C-"))
		clickElementJs(guiMap.get("recordTypeInvestigation"));
		updateHtmlReport("Choose record type", "Choose record-type pops up", "As expected", "Step", "pass", 
				"New investigation record popup");
		...
		enterText(replaceGui(guiMap.get("InvestigationField"), "Final Extension (# of days)"), "1");
		clickElementJs(guiMap.get("saveCaseButton"));
		holdSeconds(6);
		int currentTimeOut = setBrowserTimeOut(4);
		if(checkElementExists(guiMap.get("newCreateError")))
		...
		{
			setBrowserTimeOut(currentTimeOut);
			if(!checkElementExists(guiMap.get("petitionInvestigationNewCreated")))
			{
				updateHtmlReport("Create new Investigation", "User is able to create Investigation", 
						"Not as expected", "Step", "fail", "New investigation failed");
				setBrowserTimeOut(currentTimeOut);
			...
			{
				investigationId = getElementAttribute(guiMap.get("petitionInvestigationNewCreated"), "text");
				investigationId = investigationId.substring(investigationId.indexOf("I-"));
				if(caseType.equals("C-")) cInvestigation = investigationId;
				else aInvestigation = investigationId;
				highlightElement(guiMap.get("petitionInvestigationNewCreated"), "green");
				updateHtmlReport("Save the newly created investigation", "User is able to create/save investigation",
						"id: <span class = 'boldy'>"+" "+investigationId+"</span>", "Step", "pass", "New investigation created");
				setBrowserTimeOut(currentTimeOut);
				//clickElement(guiMap.get("linkNewCreatedInvestigation"));
				...
				//updateHtmlReport("Investigation details", "User is able to fill up the form", "As expected", 
						//"Step", "pass", "Investigation details");
				//holdSeconds(2);
				clickElementJs(guiMap.get("petitionInvestigationNewCreated"));
				return true;
			}
		}
	}
	...
	*/
	public static boolean createNewSegment(LinkedHashMap<String, String> row) throws Exception
	{
		...
		holdSeconds(1);
		clickElementJs(guiMap.get("SegmentFromOrderNew"));
		holdSeconds(1);
		...
		switch (segmentType)
		{
			case "Administrative Review":
			{
				...
				enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Period Covered - End Date"), 
						todayDateStr);
				enterText(replaceGui(guiMap.get("inputSegmentText"), "Number of SR Companies"), "1");
				break;
			}
		...
		clickElementJs(guiMap.get("segmentSave"));
		holdSeconds(2);
		int currentTimeOut = setBrowserTimeOut(4);
		if(checkElementExists(guiMap.get("newCreateError")))
		{
		...
		}else
		{
			...
			holdSeconds(2);
			setBrowserTimeOut(currentTimeOut);
			if(checkElementExists(guiMap.get("segmentCreated")))
			{
				///if(!segmentType.equals("Sunset Review"))
				//{
					segmentId = getElementAttribute(guiMap.get("segmentCreated"), "text");
				//}
				...
				clickElementJs(guiMap.get("segmentCreated"));
				return true;
			}else
			{
	...
	/**
	 * This method check segment outcome is mandatory if actual final sig is present
	 * @return true is error msg fired, false if not
	 * @exception Exception
	*/
	public static boolean validateReqSegmentOutcomeSunset() throws Exception
	{
		boolean works = true;
		Date todayDate = new Date();
		String todayDateStr = new SimpleDateFormat("M/d/yyyy").format(todayDate);
		//String segmentType = row.get("Segment_Type").trim();
		clickElementJs(replaceGui(guiMap.get("editSegmentIcon"), "Sunset Review") );
		clickElementJs(guiMap.get("editSegmentLink"));
		holdSeconds(1);
		switchToFrame(guiMap.get("frameOnSunsetReview"));
		clickElementJs(guiMap.get("editSunsetReview"));
		holdSeconds(2);
		selectElementByValue(guiMap.get("SelectZeroing"), "Yes");
		clickElementJs(guiMap.get("SaveSeg2"));
		holdSeconds(2);
		if (checkElementExists(guiMap.get("240DaysSection")))
		{
			updateHtmlReport("240 Days section", "The section 240 should be displayed", 
					"As expected", "VP", "pass", "240 Days section");
		}else
		{
			failTestCase("240 Days section", "The section 240 should be displayed", 
					"Not as expected", "VP", "fail", "240 Days section");
		}
		clickElementJs(guiMap.get("editSunsetReview"));
		holdSeconds(2);
		enterText(guiMap.get("SunsetReviewactFinalSig"), todayDateStr);
		clickElementJs(guiMap.get("SaveSeg2"));
		holdSeconds(2);
		scrollToElement(guiMap.get("SunsetsegOutcomeRequired"));
		if(checkElementExists(guiMap.get("SunsetsegOutcomeRequired")))
		{
			highlightElement(guiMap.get("SunsetsegOutcomeRequired"), "green");
			updateHtmlReport("Segment outcome required", "System should display "
					+ "error about missing Segment-Outcome if Actual final signature value is given", "As expected", 
							 "VP", "pass", "Segment outcome required");
			//clickElementJs(replaceGui(guiMap.get("SegSelectField"),"Segment Outcome"));
			//clickElementJs(replaceGui(guiMap.get("selectPetitionOutcomeItem"),"Completed"));
			//enterText(replaceGui(guiMap.get("inputSegmentText"), "Final Extension (# of days)"), "1");
			
		}else
		{
			works = false;
			//highlightElement(replaceGui(guiMap.get("SegSelectField"),"Segment Outcome"), "red");
			updateHtmlReport("Segment outcome required", "System should display "
					+ "error about missing Segment-Outcome if Actual final signature value is given", "Not as expected", 
							 "VP", "fail", "Segment outcome required");
		}
		switchBackFromFrame();
		clickElementJs(guiMap.get("segCancelButton"));
		return works;
	 ...
	 * @exception Exception
	*/
	public static boolean validateReqSegmentOutcome(String segmentType) throws Exception
	{
		boolean works = true;
		Date todayDate = new Date();
		String todayDateStr = new SimpleDateFormat("M/d/yyyy").format(todayDate);
		...
		switch (segmentType)
		{
			...
			case "Scope Inquiry":
			{
				clickElementJs(replaceGui(guiMap.get("SegSelectField"),"Segment Outcome"));
				holdSeconds(1);
				if(checkElementExists(replaceGui(guiMap.get("selectPetitionOutcomeItem"),"Withdrawn")))
				{
					highlightElement(replaceGui(guiMap.get("selectPetitionOutcomeItem"),"Withdrawn"), "green");
					updateHtmlReport("Check withdrawn value", "The value 'Withdrawn' should be added to the list of values"
							+ "of segment outcome", "As expected", 
									 "VP", "pass", "Check withdrawn value");
					
				}else
				{
					works = false;
					updateHtmlReport("Check withdrawn value", "The value 'Withdrawn' should be added to the list of values"
							+ "of segment outcome", "Not as expected", 
									 "VP", "fail", "Check withdrawn value");
				}
				break;
			}
			
			
			case "Anti-Circumvention Review": case "Expedited Review":
			case "Changed Circumstances Review": case "New Shipper Review":
			{
				enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Actual Final Signature"), 
						todayDateStr);
				
				if(segmentType.equalsIgnoreCase("Expedited Review") || segmentType.equalsIgnoreCase("New Shipper Review"))
				{
					enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Period Covered - Start Date"),
							todayDateStr);
					enterText(replaceGui(guiMap.get("inputInvestigationDate"), "Period Covered - End Date"), 
				...
				clickElementJs(guiMap.get("segmentSave"));
				holdSeconds(3);
				if(checkElementExists(guiMap.get("SegOutcomeRequired")))
				{
					highlightElement(guiMap.get("SegOutcomeRequired"), "green");
					updateHtmlReport("Segment outcome required", "System should display "
							+ "error about missing Segment-Outcome if Actual final signature value is given", "As expected", 
									 "VP", "pass", "Segment outcome required");
					//clickElementJs(replaceGui(guiMap.get("SegSelectField"),"Segment Outcome"));
			...
			default:
			{
				failTestSuite("Create new segment", "The type of the segment given should be valid", "Not as expected",
						"Step", "fail", "No such type of segment");
			}
		}
		return works;
	...
	/**
	 * This method check CBP Case Number field
	 * @return true is it meets req, false if not
	 * @exception Exception
	*/
	public static boolean validateCbpCaseNumberField() throws Exception
	{
		boolean works = true;
		Random rand = new Random();
		String testChain = generateRandomChars(
	            "ABCDEFGHIJKLMN OPQRSTUVWXYZ1234567890", 1+rand.nextInt(254));
		clickElementJs(replaceGui(guiMap.get("CaseIconEditField"), "CBP Case Number"));
		holdSeconds(1);
		enterTextAndClear((replaceGui(guiMap.get("CaseAnyField"), "CBP Case Number")), testChain);
		enterTextAndClear((replaceGui(guiMap.get("CaseAnyField"), "Product Short Name")), "test");
		clickElementJs(guiMap.get("CaseSave"));
		String actualVal = getElementAttribute(replaceGui(guiMap.get("caseFieldValue"), 
				"CBP Case Number"), "text");
		if(actualVal.length() == testChain.length())
		{
			highlightElement(replaceGui(guiMap.get("caseFieldValue"), 
				"CBP Case Number"), "green");
			updateHtmlReport("Save CBP case number", "Number with lenght less than 255 should be saved...<br>"
					+ "The entered value's size is "+testChain.length()+" and the saved value length is "+ actualVal.length(), 
					"As expected", "VP", "pass", "Save CBP case number less than 255 char");
		}
		else
		{
			highlightElement(replaceGui(guiMap.get("caseFieldValue"), 
			...
			updateHtmlReport("Save CBP case number", "Number with lenght less than 255 should be saved...<br>"
					+ "The entered value's size is "+testChain.length()+" and the saved value length is "+ actualVal.length(), 
					"Not as expected", "VP", "fail", "Save CBP case number less than 255 char");
		}
		
		
		//>255
		
		testChain = generateRandomChars(
	            "ABCDEFGHIJKLMN OPQRSTUVWXYZ1234567890", 256+rand.nextInt(18));
		clickElementJs(replaceGui(guiMap.get("CaseIconEditField"), "CBP Case Number"));
		holdSeconds(1);
		enterTextAndClear((replaceGui(guiMap.get("CaseAnyField"), "CBP Case Number")), testChain);
		enterTextAndClear((replaceGui(guiMap.get("CaseAnyField"), "Product Short Name")), "test");
		clickElementJs(guiMap.get("CaseSave"));
		actualVal = getElementAttribute(replaceGui(guiMap.get("caseFieldValue"), 
				"CBP Case Number"), "text");
		if(actualVal.length() == 255)
		{
			highlightElement(replaceGui(guiMap.get("caseFieldValue"), 
				"CBP Case Number"), "green");
			updateHtmlReport("Save CBP case number", "Number with lenght more than 255 should be trancated...<br>"
					+ "The entered value's size is "+testChain.length()+" and the saved value length is "+ actualVal.length(), 
					"As expected", "VP", "pass", "Save CBP case number more than 255 char");
		}
		else
		{
			highlightElement(replaceGui(guiMap.get("caseFieldValue"), 
			...
			updateHtmlReport("Save CBP case number", "Number with lenght more than 255 should be trancated...<br>"
					+ "The entered value's size is "+testChain.length()+" and the saved value length is "+ actualVal.length(), 
					"Not as expected", "VP", "fail", "Save CBP case number more than 255 char");
		}
		
		//CaseIconEditField CaseAnyField CaseSave
	...
	*/
	public static boolean createNewSegmentOld(LinkedHashMap<String, String> row) throws Exception
	{
		...
		holdSeconds(3);
		clickElementJs(replaceGui(guiMap.get("radioSegmentType"), segmentType) );
		updateHtmlReport("choosing segment type ["+segmentType+"]", "User is able to choose segment type", 
				"As Expected", "Step", "pass", "Segment "+segmentType);
		clickElementJs(guiMap.get("nextButtonSegment"));
		holdSeconds(1);
...
//		scrollToTheTopOfPage();
	...
	*/    
    ...
    public static boolean validateProgManagerNotExist(String prgMngName, boolean exists) throws Exception
    {
    	pageRefresh();
    	boolean pass=true;
    	int oldWait = setBrowserTimeOut(2);
    	if(exists)
    	{
	    	if(!elementExists(replaceGui(guiMap.get("ProgramManagerDisplayed"), prgMngName)))
	    	{
	    		pass = false;
	    		highlightElement(guiMap.get("ProgramManagerLabel"), "red");
	    		updateHtmlReport("check if program manager is displaying", "The program manger name should be"
	    				+ " displayed when login as ADCVD Manager", 
	    				"Not as expected", "VP", "fail", "Display Program Manager");
	    	}else
	    	{
	    		highlightElement(replaceGui(guiMap.get("ProgramManagerDisplayed"), prgMngName), "green");
	    		updateHtmlReport("check if program manager is displaying", "The program manger name should be"
	    				+ " displayed when login as ADCVD Manager", 
	    				"As expected", "VP", "pass", "Display Program Manager");
	    	}
    	}else //not exists
    	{
    		if(elementExists(replaceGui(guiMap.get("ProgramManagerDisplayed"), prgMngName)))
	    	{
	    		pass = false;
	    		highlightElement(replaceGui(guiMap.get("ProgramManagerDisplayed"), prgMngName), "red");
	    		updateHtmlReport("check if program manager is displaying", "The program manger name should not be"
	    				+ " displayed when login as NON-ADCVD Manager", 
	    				"Not as expected", "VP", "fail", "Display Program Manager");
	    	}else
	    	{
	    		highlightElement(guiMap.get("ProgramManagerLabel"), "green");
	    		updateHtmlReport("check if program manager is displaying", "The program manger name should not be"
	    				+ " displayed when login as NON-ADCVD Manager", 
	    				"As expected", "VP", "pass", "Display Program Manager");
	    	}
    	}
    	setBrowserTimeOut(oldWait);
    	return pass;
    }
    
    public static void navigateToObject(String baseUrl, String recId)
    {
    	String recURL = baseUrl+"/"+recId;
    	navigateTo(recURL);
    }
}
