...
public class FTZFormLib{
									 ...
									 String password) throws Exception
	{
		...
		}else
		{
			...
			if(checkElementExists(replaceGui(guiMap.get("TopMenu"), "Sign out")))
			{
				...
				clickElementJs(replaceGui(guiMap.get("TopMenu"), "Case") );
	 ...
	 * @exception Exception
	*/
	public static boolean submitFtzCase(HashMap<String, String> row) throws Exception
	{
		boolean newForm = true;
		String elementName = "";
		clickElementJs(replaceGui(guiMap.get("TopMenu"), "New Case") );
		for (HashMap.Entry<String, String> entry : row.entrySet()) 
		{
			...
			{
				continue;
			}else if (elementName.equalsIgnoreCase("Case Status") || elementName.equalsIgnoreCase("Analyst Assigned")||
					elementName.equalsIgnoreCase("Other Analyst Assigned") || elementName.equalsIgnoreCase("State"))
			{
				selectElementByText(replaceGui(guiMap.get("selectField"), elementName), entry.getValue());
			}else if (elementName.equalsIgnoreCase("IsActive"))
			{
				if(entry.getValue().equalsIgnoreCase("No"))
					clickElementJs(guiMap.get("isActive"));
			}else if (elementName.equalsIgnoreCase("Case Type Group"))
			{
				clickElementJs(replaceGui(guiMap.get("BroadStaff"), 
						entry.getValue().equalsIgnoreCase("Board")? "B":"S"));
				
			}else if (elementName.equalsIgnoreCase("Docket #"))
			{
				if (!entry.getValue().trim().equalsIgnoreCase("") && !entry.getValue().trim().equalsIgnoreCase(""))
				{
