...
public class ExporterLib{
	 ...
	 * @exception Exception
	*/
	public static String createContent(LinkedHashMap<String, String> row) throws Exception
	{
		...
		if(!"N/A".equalsIgnoreCase(row.get("Content_Public_Title"))
				&& !"".equalsIgnoreCase(row.get("Content_Public_Title")))
		{
		...
		{
			enterText(guiMap.get("TradeLeadContentTitle"), row.get("Trade_Lead_Title"));
		}
		...
		String allStatus[] = getSelectValues(guiMap.get("SaveAs"));
		if(!arrayContainsValue(allStatus, "Ready for Approval"))
		{
			failTestCase("Checking statuses", "The status [Ready for Approval] shoud be in the list of options",
					"Not as expected", "VP", "fail", "");
		}else
		{
			updateHtmlReport("Checking statuses", "The status [Ready for Approval] shoud be in the list of options",
					"As expected", "VP", "pass", "");;
		}
		selectElementByText(guiMap.get("SaveAs"), "Ready for Approval");
		...
		{
			GuiTools.testCaseStatus = false;
			highlightElement(guiMap.get("ContentCreated"), "red");
			failTestCase("Create content ["+type+"]",  "User is able to create ["+type+"]", "Not as expected", 
	...
	/**
	 * This method approve content
	 * @param articleTitle: Title of the article to be approved
	 * @param type: Type of the article to be approved
	 * @return true content approved correctly, false if not
	 * @exception Exception
	*/
	public static void approveArticle(String articleTitle, String type) throws Exception
	{
		//int currentWait = setBrowserTimeOut(2);
		if(!elementExists(guiMap.get("ContentMenu")))
		{
		...
		clickNiemElementJs(guiMap.get("ContentMenu"), 1);
		enterText(guiMap.get("editTitle"), articleTitle);
		clickElementJs(guiMap.get("filterButton"));
		clickNiemElementJs(guiMap.get("editButton"), 1);
		
		String allStatus[] = getSelectValues(guiMap.get("SaveAs"));
		if(!arrayContainsValue(allStatus, "Ready to Publish"))
		{
			failTestCase("Checking statuses", "The status [Ready to Publish] shoud be in the list of options",
					"Not as expected", "VP", "fail", "");
		}else
		{
			updateHtmlReport("Checking statuses", "The status [Ready to Publish] shoud be in the list of options",
					"As expected", "VP", "pass", "");;
		}
		selectElementByText(guiMap.get("SaveAs"), "Ready to Publish");
		clickElementJs(guiMap.get("ButtonSave"));
		if(checkElementExists(guiMap.get("statusUpdated")))
		{
			highlightElement(guiMap.get("statusUpdated"), "green");
			updateHtmlReport("Approve content ["+type+"]",  "User is able to approve ["+type+"]", "As expected", 
					"Step", "pass", "Create content "+type);
		}else
		{
			GuiTools.testCaseStatus = false;
			highlightElement(guiMap.get("statusUpdated"), "red");
			failTestCase("Approve content ["+type+"]",  "User is able to approve ["+type+"]", "Not as expected", 
					"Step", "fail", "Approve content "+type);
	...
	/**
	 * This method publish content
	 * @param articleTitle: Title of the article to be approved
	 * @param type: Type of the article to be approved
	 * @return true content approved correctly, false if not
	 * @exception Exception
	*/
	public static void publishArticle(String articleTitle, String type) throws Exception
	{
		if(!elementExists(guiMap.get("ContentMenu")))
		{
			clickElementJs(guiMap.get("MainMenu"));
		...
		clickNiemElementJs(guiMap.get("editButton"), 1);
		String allStatus[] = getSelectValues(guiMap.get("SaveAs"));
		if(!arrayContainsValue(allStatus, "Published"))
		{
			failTestCase("Checking statuses", "The status [Published] shoud be in the list of options",
					"Not as expected", "VP", "fail", "");
		}else
		{
			updateHtmlReport("Checking statuses", "The status [Published] shoud be in the list of options",
					"As expected", "VP", "pass", "");;
		}
		selectElementByText(guiMap.get("SaveAs"), "Published");
		clickElementJs(guiMap.get("ButtonSave"));
		if(checkElementExists(guiMap.get("statusUpdated")))
		{
			highlightElement(guiMap.get("statusUpdated"), "green");
			String readSts = getElementAttribute(replaceGui(guiMap.get("displayedStatus"), articleTitle), "text");
			if(readSts.trim().equalsIgnoreCase("Published"))
			{
				highlightElement(replaceGui(guiMap.get("displayedStatus"), articleTitle), "green");
				updateHtmlReport("Publish content ["+type+"]",  "User is able to publish ["+type+"]", "As expected", 
						"Step", "pass", "Create content "+type);
			}
			else
			{
				GuiTools.testCaseStatus = false;
				highlightElement(replaceGui(guiMap.get("displayedStatus"), articleTitle), "red");
				updateHtmlReport("Publish content ["+type+"]",  "User is able to publish ["+type+"]", "Not as expected", 
						"Step", "fail", "Create content "+type);
			}
		}else
		{
			GuiTools.testCaseStatus = false;
			highlightElement(guiMap.get("statusUpdated"), "red");
			failTestCase("Publish content ["+type+"]",  "User is able to publish ["+type+"]", "Not as expected", 
					"Step", "fail", "Create content "+type);
	...
	*/
	...
	/**
	 * This method checks if an array contains particular value
	 * @param array: given string
	 * @param v: The value
	 * @return true if array contains v, else if not
	 * @exception Exception
	*/
	
	public static <T> boolean arrayContainsValue(final T[] array, final T v) {
	    if (v == null) {
	        for (final T e : array)
	            if (e == null)
	                return true;
