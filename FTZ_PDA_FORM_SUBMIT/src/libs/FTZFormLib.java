...
public class FTZFormLib{
									 ...
									 String password) throws Exception
	{
		...
		}else
		{
			...
			if(checkElementExists(replaceGui(guiMap.get("TopMenu"), "Sign out")))
			{
				...
				clickElementJs(replaceGui(guiMap.get("TopMenu"), "PDA") );
	...
	*/
	public static boolean submitFtzCase(HashMap<String, String> row) throws Exception
	{
		...
		updateHtmlReportOverall("Fill up the PDA form",  "User is able to fill up the form", "As expected", 
		...
		{
			setBrowserTimeOut(currentWait);
			updateHtmlReport("Submit new PDA form",  "User is able to submit PDA form", "As expected", 
				"Step", "pass", "crete new PDA form");
		}else
		{
			newForm = false;
			setBrowserTimeOut(currentWait);
			updateHtmlReport("Submit new PDA form",  "User is able to submit PDA form", "Not as expected", 
					"Step", "fail", "crete new PDA form");
		}
		return newForm;
	}
